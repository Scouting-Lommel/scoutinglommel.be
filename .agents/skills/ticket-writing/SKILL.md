...
7. **If a plan was created, attach the entire plan as a Linear document to the ticket.** When a work plan exists (e.g. `.omo/plans/*.md`), create a Linear document containing the plan's full content — in its entirety, never condensed or summarized — and link it to the ticket. The ticket description stays the contract; the document preserves the full plan (tasks, acceptance criteria, QA scenarios, dependency matrix, commit strategy) so it is accessible from any machine via Linear. Do this before starting implementation.
...
## Plan attachment (when a plan file exists)

If a work plan was created for the ticket (e.g. `.omo/plans/<name>.md`):

- Create a Linear document with the plan's **full content** — the entire file, verbatim, never a condensed summary.
- Link the document to the ticket (the document's parent issue is the ticket; for a parent ticket with sub-issues, link it to the parent).
- Title the document after the plan (e.g. "Monorepo Migration Plan").
- Do this **before starting implementation**, so the plan is preserved in Linear and accessible from any machine — the local `.omo/plans/` file is not a substitute.

The ticket description stays the contract (Goal / What to do / Acceptance criteria / Out of scope); the document is the full plan archive.
